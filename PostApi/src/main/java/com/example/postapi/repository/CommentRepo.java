...
public interface CommentRepo extends ReactiveCrudRepository<Comment,Long> {
    ...
    Mono<Comment> saveComment(Long post_id,Long user_id, String text);
    @Async
    @Query("DELETE FROM public.comment_post WHERE post_id=:post_id and user_id = :user_id and text = :text; ")
    Mono<Void> deleteComment(Long post_id,Long user_id, String text);
    ...
    Flux<Comment> findAllByManyPosts(List<Long> posts_id);
    @Async
    ...
    Flux<Comment> findAllByManyPosts(List<Long> posts_id, long start);
